...
| `month-scope.test.js` | **Where an edit lands.** Settings holds the figure that sticks; an amount typed on the Bills or Income tab belongs to the month that was on screen and never reaches the record. Also that a later Settings change still reaches every month that didn't override it. |
